...
| `code-review-expert` | Thermo-nuclear branch review: deep correctness and security audits plus harsh code-quality rubrics, parallel subagents, code-review-expert orchestration, and optional take-the-wheel and FSD merge-ready flows. |
...
| `gh-expert` | Expert GitHub CLI plugin containing 18 skills for PR management, issue tracking, releases, repos, codespaces, gists, projects, workflows, search, labels, secrets, discussions, orgs, rulesets, Copilot CLI, config, browse, and auth automation. |
| `git-expert` | Expert git plugin containing 25 skills for advanced version control: commit, bisect, merge, worktree, rebase, stash, revert, cherry-pick, tag, remote, log, blame, reflog, clean, reset, config, hooks, push, pull, fetch, diff, branch, switch, clone, and submodule automation. |
...
| `karpathy-guidelines` | > |
| `karpathy-loop` | Autonomous Loop Engineering engine based on Andrej Karpathy's Autoresearch method. Enables AI agents to run iterative hypothesis-modify-execute-validate-improve cycles for continuous optimization of code, models, and metrics with CAG + Prompt Cache strategy. |
